...
public class View_RailFence {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					View_RailFence window = new View_RailFence();
	...
	private void initialize() {
		...
		JLabel lblNewLabel = new JLabel("Rail Fence");
		lblNewLabel.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel.setFont(new Font("Tahoma", Font.BOLD, 16));
		lblNewLabel.setBounds(0, 10, 433, 24);
	...
	private void btn_encryptActionPerformed(ActionEvent evt) {
		try {
			...
			String ct = RailFenceCipher.encrypt(plaintext, k);
	...
	private void btn_decryptActionPerformed(ActionEvent evt) {
		try {
			...
			int k = Integer.parseInt(this.key.getText());
			String ct = RailFenceCipher.decrypt(ciphertextToDe, k);
