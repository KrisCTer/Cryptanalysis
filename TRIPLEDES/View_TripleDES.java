...
public class View_TripleDES {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					View_TripleDES window = new View_TripleDES();
	...
	private void initialize() {
		...
		JLabel lblNewLabel = new JLabel("TripleDES");
	...
	private void btn_encryptActionPerformed(ActionEvent evt) {
		try {
			...
			String secretKey = this.key.getText();
			if (secretKey.length() == 24) {
				String ct = TripleDESCipher.encrypt(plaintext, secretKey);
				cipherText.setText(ct);
				this.saveToFile(ct);
				JOptionPane.showMessageDialog(this.frame, "Encrypt successfully");
			} else {
				JOptionPane.showMessageDialog(this.frame, "Secret key must be 24 characters long", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		} catch (InvalidKeyException | NoSuchAlgorithmException | BadPaddingException | IllegalBlockSizeException
				| NoSuchPaddingException e) {
			JOptionPane.showMessageDialog(this.frame, "Error" + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void btn_decryptActionPerformed(ActionEvent evt) {
		try {
			...
			String secretKey = this.key.getText();
			if (secretKey.length() == 24) {
				String ct = TripleDESCipher.decrypt(cipherText, secretKey);
