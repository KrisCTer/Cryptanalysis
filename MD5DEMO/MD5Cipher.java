...
public class MD5Cipher {
	...
	// Bảng s định nghĩa các giá trị dịch bit cho từng vòng.
	...
	// Bảng K chứa các hằng số lấy từ giá trị sin của các số nguyên.
	...
	// Hàm xoay trái (leftrotate) một giá trị 32-bit.
	...
	public static String computeMD5(String message) {
		...
		paddedMsg[msgBytes.length] = (byte) 0x80; // Thêm bit '1'.

		// Thêm độ dài thông điệp (64-bit, little-endian).
		for (int i = 0; i < 8; i++) {
			paddedMsg[paddedMsg.length - 8 + i] = (byte) (originalLengthBits >>> (8 * i));
		}

		// Bước 3: Khởi tạo các giá trị A, B, C, D.
		int a0 = 0x67452301;
		int b0 = 0xefcdab89;
		int c0 = 0x98badcfe;
		int d0 = 0x10325476;

		// Bước 4: Xử lý từng khối 512-bit.
		for (int offset = 0; offset < paddedMsg.length; offset += 64) {
			...
			// Cộng kết quả của khối này vào giá trị băm hiện tại.
			a0 += A;
			b0 += B;
			c0 += C;
			d0 += D;
		...
		// Bước 5: Xuất giá trị băm cuối cùng (little-endian).
